...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON %I', policy_record, table_name);
        RAISE NOTICE 'Dropped policy: %', policy_record;
    END LOOP;
    
    -- Ensure any pre-existing policies are removed then recreate simple, non-recursive policies
    EXECUTE format('DROP POLICY IF EXISTS usuarios_select_own ON %I', table_name);
    EXECUTE format('DROP POLICY IF EXISTS usuarios_select_authenticated ON %I', table_name);
    EXECUTE format('DROP POLICY IF EXISTS usuarios_update_own ON %I', table_name);
    EXECUTE format('DROP POLICY IF EXISTS usuarios_all_service_role ON %I', table_name);
    EXECUTE format('
...
BEGIN
    ...
    -- Create simple policies for usuarios_roles (DROP first to avoid duplicates)
    DROP POLICY IF EXISTS usuarios_roles_select ON usuarios_roles;
...
BEGIN
    ...
    -- Create simple policies for usuarios_grupos (DROP first to avoid duplicates)
    DROP POLICY IF EXISTS usuarios_grupos_select ON usuarios_grupos;
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON usuarios_asignaciones', policy_record);
        RAISE NOTICE 'Dropped policy: %', policy_record;
    END LOOP;
    
    -- Create simple policies for usuarios_asignaciones (ensure fresh)
    DROP POLICY IF EXISTS usuarios_asignaciones_select ON usuarios_asignaciones;
    DROP POLICY IF EXISTS usuarios_asignaciones_all ON usuarios_asignaciones;
...
DO $$
BEGIN
    -- Recreate compatibility view for legacy function name to avoid duplicates
    EXECUTE 'DROP VIEW IF EXISTS public.asignaciones_usuario';
...
-- Ensure FK for usuarios.id_suc -> sucursales.id_suc so Supabase schema cache can resolve relationships
DO $$
DECLARE
    exists_fk_suc boolean := false;
BEGIN
    SELECT EXISTS(
        SELECT 1 FROM information_schema.table_constraints tc
        JOIN information_schema.key_column_usage kcu ON tc.constraint_name = kcu.constraint_name
    ...
    IF NOT exists_fk_suc THEN
        RAISE NOTICE 'Adding FK usuarios.id_suc -> sucursales.id_suc';
        EXECUTE 'ALTER TABLE public.usuarios ADD CONSTRAINT fk_usuarios_id_suc FOREIGN KEY (id_suc) REFERENCES public.sucursales(id_suc)';
    ELSE
        RAISE NOTICE 'Foreign key usuarios.id_suc -> sucursales.id_suc already exists';
    END IF;
END $$;
