...
BEGIN
    ...
    INSERT INTO public.ddl_migrations_log (mensaje) 
    VALUES ('ROLE_ID_VALIDATION: Found ' || v_role_count || ' roles, ' || v_user_count || ' users, ' || v_assignment_count || 
            ' assignments. Backups: ' || v_backup_role_count || ' roles, ' || v_backup_user_count || ' users, ' || v_backup_assignment_count || ' assignments');
...
BEGIN
    ...
    INSERT INTO public.ddl_migrations_log (mensaje) 
    VALUES ('ROLE_ID_FK_CONSTRAINTS: Found ' || COALESCE(array_length(constraint_names, 1)::text, '0') || 
            ' constraints to handle: ' || array_to_string(constraint_names, ', '));
END $$;
...
EXCEPTION WHEN OTHERS THEN
    INSERT INTO public.ddl_migrations_log (mensaje) VALUES ('ROLE_ID_FK_ERROR: Failed to recreate fk_asignacion_rol: ' || SQLERRM);
    RAISE;
END $$;
...
BEGIN
    ...
    INSERT INTO public.ddl_migrations_log (mensaje) 
    VALUES ('ROLE_ID_VALIDATION_SUCCESS: All ' || v_roles_with_correct_ids || ' roles migrated. Users: ' || 
            v_users_with_correct_roles || ', Assignments: ' || v_assignments_with_correct_roles || '. Total roles: ' || v_new_role_count);
