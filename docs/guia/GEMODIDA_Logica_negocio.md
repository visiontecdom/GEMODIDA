...
DISEÑO Y MANEJO DE ENCUESTAS
Las encuestas son una de las principales actividades que realiza el departamento de monitoreo de la DIDA. Es un proceso mediante el cual se recogen datos de diferentes tipos y se registran para organizarlos y generar informes estatisticos.
Plantillas para encuestas
El sistema tendrá una sección especializada en el diseño de plantillas para realizar encuestas. Las plantillas serán modelos con campos de distintos tipos (Fecha, Texto, Numérico, Correo, Telefono, Etc.)  y con todas las características especificadas por el usuario, para realizar captura de datos, ya sean estos escritos por un operador de campo, o recuperados haciendo scraping en internet, o que sean llenados por usuarios particulares a través de las redes sociales.
Todas las plantillas de encuestas y de sondeos serán reutilizables y duplicables para crear nuevas plantillas. Y para que puedan ser utilizadas en los trabajos de campo, deberán ser marcadas como ‘Terminada’ por un usuario con nivel de acceso administrativo.
Las plantillas de encuestas serán diseñadas con un estilo moderno, dinámico y fácil de usar para cualquier persona con pocos conocimientos de informática o tecnología.
Realización de las encuestas
Los usuarios con nivel ‘operador’ o ‘gestor’ en el sistema, tendrán acceso a utilizar el panel de encuestas.
Desde el panel de encuestas se podrá navegar a la pagina de realizar encuestas, donde el usuario deberá elegir el tipo de encuesta que va a realizar y posteriormente podrá empezar a registrar los datos de la encuesta vinculada a esa plantilla.
...
Este panel/dashboard será mostrado a los usuarios con rol de 'desarrollo' y 'super_user'
Tendrá funcionalidades diversas que se irán organizando durante el proceso de desarrollo para uso de diferentes tareas administrativas y de soporte.

INTERFAZ OPERATIVA
En la página de bienvenida/inicio, vamos a crear un contenedor con un conjunto de tarjetas/botones que tengan las siguientes características:
	1- Titulo (Visible en primera línea. Texto en negrita. Es lo que identifica la acción)
	2- Descripción (Visible en segunda línea. Texto normal. Es un detalle ampliado y breve de la acción sobre lo que dice el titulo)
	3- Grupo (No visible al usuario. Solo para uso interno del sistema. Indicará el grupo de usuarios al cual va a pertenecer la acción para saber cuál será el espacio de trabajo del usuario)
	- Enlace (No visible al usuario. Solo para uso del sistema. Indicará la URL del panel o la página a donde debe navegar el sistema)
	- Posición (No visible al usuario. solo para uso interno del sistema. Representa el nivel de jerarquía que ocupa el botón/tarjeta en el contenedor)
	- Icono (Visible al usuario. Colocado encima de la primera línea de texto y centrado. Es una imagen que va a representar la opción en el botón/tarjeta)
	
Cada una de las tarjetas/botones de ese contenedor van a mostrar los siguientes títulos y descripciones:
1. GESTION DE MONITOREO: Acceso al sistema de gestión de monitoreo a la calidad de la seguridad social)
2. GESTION DE PROMOCIONES: Navega al sistema para la gestión de promociones y actividades de la DIDA
3. SOPORTE Y MANTENIMIENTO: Asistencia de usuarios, resolución de dificultades y configuraciones.
4. OTRAS TAREAS: Esta opción quedará con un status 'En desarrollo...', pendiente para agregarle funcionalidades en el futuro.

Tras hacer clic en una de esas tarjetas/opciones de ese contenedor, el usuario será dirigido a la página de iniciar sesión en la aplicación.

Cuando el usuario inicie sesión, el sistema debe recuperar toda la información de la base de datos necesaria para identificar a cuál grupo de trabajo pertenece, en cuales sucursales tiene acceso, cuál es su rol de usuario y cuales permisos tiene ese rol para trabajar en el sistema.

Para lograr esa dinámica en la lógica de seguridad, vamos a revisar las tablas existentes en la base de datos, a fin de saber si son suficientes o si se requiere agregar nuevas tablas o nuevas columnas a las tablas que ya existen, para lograr que se cumplan los criterios ya mencionados.

Luego de recuperar las informaciones del usuario, grupo, sucursal, rol y permisos, el sistema debe mostrar el panel de trabajo que le corresponda a ese usuario, de acuerdo a lo que ya se ha definido en la lógica de negocio de este proyecto.
